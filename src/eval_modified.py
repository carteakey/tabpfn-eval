...
classifier = TabPFNClassifier(device='cuda', base_path='/home/kchauhan/repos/mds-tmu-dl/src/models/tabpfn/modified', model_string='kc',N_ensemble_configurations=32,seed=42)
...
for did in tqdm(openml_list.index):
    ...
    print(entry)
    try:
        X, y, categorical_feats, attribute_names = get_openml_classification(int(entry.id), max_samples = 2000,multiclass=True, shuffled=True)
    except:
        continue
...
cross_entropy_list = [s["cross_entropy"] for _, s in scores.items() if s["cross_entropy"] is not None]
